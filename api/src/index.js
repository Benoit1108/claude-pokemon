...
// Cache-Control: 5min freshness vs hammer protection.

const LINEAGE_LABELS = {
  fire: "Feu", water: "Eau", grass: "Plante", electric: "Électrik", eevee: "Eevee",
  chikorita: "Plante (Johto)", cyndaquil: "Feu (Johto)", totodile: "Eau (Johto)",
};
const LINEAGE_EMOJI = {
  fire: "🔥", water: "💧", grass: "🌿", electric: "⚡", eevee: "🦊",
  chikorita: "🌱", cyndaquil: "🦔", totodile: "🐊",
};
const LINEAGE_ACCENT = {
  fire: "#ef6c00", water: "#268fff", grass: "#64b437", electric: "#ffda00",
  eevee: "#c2a88a", chikorita: "#7eb858", cyndaquil: "#e8a32a", totodile: "#3d8de8",
};
...
function svgPlaceholder(message) {
...
async function handleBadge(pathname, env) {
  // Path is /v1/badge/<anon_id>.svg
  const m = pathname.match(/^\/v1\/badge\/([a-f0-9]{8,16})\.svg$/);
  if (!m) {
    return new Response(svgPlaceholder("invalid badge URL"), {
      status: 400,
      headers: { "content-type": "image/svg+xml", ...corsHeaders() },
    });
  }
  const anon_id = m[1];
  const raw = await env.STATS.get(`stats:${anon_id}`);
  if (!raw) {
    return new Response(svgPlaceholder("trainer not found"), {
      status: 404,
      headers: {
  ...
  catch { return new Response(svgPlaceholder("data corrupt"), { status: 500 }); }

  return new Response(svgBadge(record), {
    status: 200,
    headers: {
      "content-type": "image/svg+xml",
      "cache-control": "public, max-age=300",  // 5 min — refresh-friendly
      ...corsHeaders(),
    },
  });
}
