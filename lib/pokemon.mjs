...
function renderTeam(ctx) {
  ...
  if (pcCount > 0) {
    out2 += bashPrintf(`  %s${t(ctx.locale, "team.pc_overflow")} \u2014 %sbash %s pc%s
    ...
    for (const ev of events.slice(0, 3)) {
      ...
      switch (ev.type) {
        ...
        case "encounter":
          ...
          break;
        case "battle_won":
          out2 += bashPrintf(`   \u2694\uFE0F  ${t(locale, "battle.won", wildName(eid), exp)}   %s%s%s
`, DIM, at, RESET);
          break;
        case "battle_lost":
          out2 += bashPrintf(`   \u{1F494} ${t(locale, "battle.lost", wildName(eid))}   %s%s%s
`, DIM, at, RESET);
          break;
        case "item":
...
`, DIM, RESET);
  out2 += bashPrintf(`  %s${tPad(locale, "common.example", 22)}%s : %sbash ~/.claude/pokemon-status.sh team%s
