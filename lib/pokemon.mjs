...
var SECRET_FILE = join(POKEMON_DIR, ".arena-secret");
var SESSION_FILE = join(POKEMON_DIR, ".session");
...
function writeSecretFile(path, value) {
  writeFileSync(path, value, { mode: 384 });
}
function rm(path) {
  try {
    unlinkSync(path);
...
var nowEpoch = process.env.POKEMON_NOW_EPOCH ? Number(process.env.POKEMON_NOW_EPOCH) : Math.floor(Date.now() / 1e3);
var nowIso = new Date(nowEpoch * 1e3).toISOString().replace(/\.\d{3}Z$/, "Z");
