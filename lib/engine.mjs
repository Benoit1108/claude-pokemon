...
function rosterEntry(entry, slot, marker, data, locale) {
  ...
  const star = entry.is_shiny === true ? `${GOLD7}\u2605${RESET7} ` : "";
  const label = data.lineages?.[lin]?.label ?? lin;
  const markerStr = marker === "active" ? `  ${GOLD7}${t(locale, "common.active_marker")}${RESET7}` : "";
  return bashPrintf("   %s[%s]%s  %s%-22s  %sLv.%d%s  %s%s%s%s\n", BOLD7, slot, RESET7, star, name, BOLD7, lvl, RESET7, DIM7, label, RESET7, markerStr);
}
...
`, DIM7, RESET7);
      ...
      for (let i = 0; i < team2.length; i++)
        out += rosterEntry(team2[i], String(i), "", data, locale);
      out += bashPrintf(`
  %s${L("switch.usage")}%s
...
`, DIM7, RESET7), state, stateChanged: false };
  }
...
`, DIM7, RESET7), state: next2, stateChanged: true };
  }
  if (rawAnswer === "") {
    if (currentQuizId !== "") {
...
`, DIM7, RESET7) + gameHints(data, lang, idx2, locale));
    }
...
`, DIM7, RESET7), state, stateChanged: false };
  ...
  let lvlStr = `Lv.${level}`;
  if (shiny)
    lvlStr = `${GOLD7}\u2605${RESET7} ${lvlStr}`;
  out += bashPrintf("  %s#%03d %s %s%s   %s%s%s\n", BOLD7, dex, name, lvlStr, shinyStr, DIM7, `(par ${trainer})`, RESET7);
...
async function main() {
  ...
  if (!handler && !isRender && !isMutate && !isTick && !isNet && !isConfig && !isShare && !isArena && !isLogin && !isLogout && !isCmd) {
