...
function tick(input) {
  ...
  const xpMultiplier2 = xpMultFor(usedPct);
  ...
  let weightedDelta = 0;
  if (!shinyHunter) {
    let m = xpMultiplier2 * typeMatch * dailyMult * statusMult * heldMult * injuredMult * seasonMult;
    if (m > 2)
      m = 2;
    weightedDelta = Math.trunc(delta * m);
  }
  s.last_xp_multipliers = {
    context: xpMultiplier2.toFixed(1),
