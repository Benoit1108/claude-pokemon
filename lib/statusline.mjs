...
function trimSprite(content) {
  ...
  for (let i = first; i <= last; i++) {
    let rest = lines[i].replace(/\x1b\[[?]25[lh]/g, "");
    let kept = "";
    let strip = minLead;
...
function renderInline(state, data) {
  ...
  if (color === "dim" && !shinyColor)
    gaugeColor = BRIGHT_CYAN;
  const LEVEL_COLOR = DIM + WHITE;
  if (flash > 0) {
    const sparkle = `${BRIGHT_YELLOW}\u2728${RESET}`;
    return `${shinyPrefix}${colorCode}${BOLD}${emoji} ${sparkle}${name}${sparkle}${RESET} ${LEVEL_COLOR}Lv.${level}${RESET} ${colorCode}${xpLabel}${RESET}/${DIM}${nextLabel}${RESET} ${gaugeColor}${gaugeFilled}${DIM}${gaugeEmpty}${RESET} ${pctColor}${pct}%${RESET}`;
  }
  return `${shinyPrefix}${colorCode}${BOLD}${emoji} ${name}${RESET} ${LEVEL_COLOR}Lv.${level}${RESET} ${colorCode}${xpLabel}${RESET}/${DIM}${nextLabel}${RESET} ${gaugeColor}${gaugeFilled}${DIM}${gaugeEmpty}${RESET} ${pctColor}${pct}%${RESET}`;
}
...
`;
  ...
  if (used !== "") {
    ...
    out += `  ${gaugeColor}[${bar}]${RESET} ${DIM}${usedInt}%${RESET} ${multColor}\xD7${mult}${RESET}`;
  }
  if (project !== "")
