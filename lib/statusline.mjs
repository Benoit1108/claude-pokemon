...
function computeDecisions(state, data) {
  ...
  return {
    ...
    berry: { fired: Math.random() < Number(data.event_chances?.berry ?? 5e-3), index: bl ? Math.floor(Math.random() * bl) : 0 },
    encounter: { fired: Math.random() < Number(data.event_chances?.encounter ?? 1e-3), index: wl ? Math.floor(Math.random() * wl) : 0 },
    battle: {
