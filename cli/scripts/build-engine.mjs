...
// resolve to real files. Run AFTER `npm run -w cli build`.
//   node cli/scripts/build-engine.mjs
...
const common = {
  ...
  // byte-identical no matter the invoking CWD (repo root vs the `cli`
  // workspace). Without this, the `// <path>.js` markers flip between
  // `cli/dist/…` and `dist/…` and the committed-bundle drift check fails.
  ...
  banner: {
    js: '// GENERATED by cli/scripts/build-engine.mjs — do not edit. Run `npm run build:data`.',
  },
  legalComments: 'none',
}
...
await build({
  ...common,
  entryPoints: [join(cliRoot, 'dist', 'statusline-entry.js')],
...
await build({
  ...common,
  entryPoints: [join(cliRoot, 'dist', 'pokemon-entry.js')],
