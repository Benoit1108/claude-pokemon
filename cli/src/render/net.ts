...
export function renderLeaderboard(
  ...
  out += bashPrintf(
    `  %s${t(locale, 'leaderboard.subtitle', resp.total_players)}%s\n\n`,
    DIM,
    RESET,
  )
...
export function renderAggregate(_data: PokemonData, locale: Locale, result: NetResult): string {
  ...
  out += bashPrintf(
    `  %s${tPad(locale, 'aggregate.players', 22)}%s :  %s\n`,
    DIM,
    RESET,
    fmtInt(players),
  )
  out += bashPrintf(
    `  %s${tPad(locale, 'aggregate.tokens', 22)}%s :  %s\n`,
    DIM,
    RESET,
    fmtInt(resp.total_tokens_combined),
  )
  out += bashPrintf(
    `  %s${tPad(locale, 'aggregate.shinies', 22)}%s :  %s\n`,
    DIM,
    RESET,
    fmtInt(resp.total_shinies_observed),
  )
  out += bashPrintf(
    `  %s${tPad(locale, 'aggregate.shiny_rate', 22)}%s :  %s\n\n`,
    DIM,
    RESET,
    resp.shiny_rate_observed ?? 0,
  )
