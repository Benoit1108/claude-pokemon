...
import { type CommandInput, type CommandResult, type Translator, makeTranslator, cloneState, wildName } from './shared.js'
...
export function cmdGame(input: CommandInput): CommandResult {
  ...
  if (lineage === '' || lineage === null) return noChange(bashPrintf(`  %s${tr('game.no_active')}%s\n\n`, DIM, RESET))

  const currentQuizId = state.current_quiz?.id ?? ''
  if (rawAnswer === 'skip') {
    if (currentQuizId === '') return noChange(bashPrintf(`  %s${tr('game.no_quiz')}%s\n\n`, DIM, RESET))
    const next = cloneState(state)
    delete next.current_quiz
    return { output: title + bashPrintf(`  %s${tr('game.skipped')}%s\n\n`, DIM, RESET), state: next, stateChanged: true }
  }
