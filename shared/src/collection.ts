...
function lastStageName(state: Json): string {
  const hist: Json[] = Array.isArray(state.evolution_history) ? state.evolution_history : []
...
// left untouched.
export function activeToArchive(state: Json, now: string): Json {
  ...
  const completed: Json[] = Array.isArray(s.lifetime_stats.lineages_completed)
...
export function resetActive(state: Json, now: string, forcedLineage?: string | null): Json {
  const s = clone(state)
...
export function archiveToTeam(state: Json, now: string): Json {
  return resetActive(activeToArchive(state, now), now)
}

function loadEntryToActive(s: Json, entry: Json, now: string): void {
...
export function loadTeamToActive(state: Json, now: string, idx: number): Json {
  const s = clone(state)
...
export function teamToPc(state: Json, idx: number): Json {
  const s = clone(state)
  s.pc_storage = Array.isArray(s.pc_storage) ? s.pc_storage : []
...
export function pcToTeamOrActive(state: Json, now: string, idx: number): Json | null {
...
export function releaseSlot(state: Json, area: string, idx: number): Json {
  const s = clone(state)
