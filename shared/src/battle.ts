...
const TYPE_CHART_OVERRIDES: Partial<Record<CombatType, Partial<Record<CombatType, number>>>> = {
...
function buildTypeChart(): Record<CombatType, Record<CombatType, number>> {
  const chart = {} as Record<CombatType, Record<CombatType, number>>
  for (const attacker of COMBAT_TYPES) {
    chart[attacker] = {} as Record<CombatType, number>
...
export const TYPE_CHART: Record<CombatType, Record<CombatType, number>> = buildTypeChart()
