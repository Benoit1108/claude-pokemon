...
export function thresholdFor(thresholds, level) {
    return thresholds[level];
}
/** Highest level whose cumulative threshold is ≤ totalXp. */
export function levelFromXp(thresholds, totalXp) {
    let count = 0;
...
export function xpToNext(thresholds, totalXp, level) {
...
export function progressPct(thresholds, totalXp, level) {
    const maxLevel = thresholds.length - 1;
    if (level >= maxLevel)
        return 100;
 ...
 */
export function xpMultiplier(usedPct) {
    if (usedPct == null)
        return 1.0;
 ...
 */
export function typeMatchMultiplier(lineage, usedPct) {
    const p = Math.round(usedPct);
    switch (lineage) {
        case 'fire':
            return p < 30 ? 1.2 : 1.0;
