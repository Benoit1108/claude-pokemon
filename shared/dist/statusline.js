...
export function ansiColor(theme, name) {
    if (theme === 'retro') {
        switch (name) {
            case 'dim':
                return '\x1b[38;5;22m';
...
export function rainbowName(name) {
    const rainbows = ['\x1b[91m', '\x1b[93m', '\x1b[92m', '\x1b[96m', '\x1b[94m', '\x1b[95m'];
    let out = '';
    const chars = [...name];
    for (let i = 0; i < chars.length; i++)
        out += rainbows[i % rainbows.length] + chars[i];
    return out;
}
...
export function trimSprite(content) {
    const lines = content.split('\n');
    if (lines.length && lines[lines.length - 1] === '')
        lines.pop();
    const stripAnsi = (s) => s.replace(/\x1b\[[?0-9;]*[a-zA-Z]/g, ''); // eslint-disable-line no-control-regex
    let first = -1;
    let last = -1;
    let minLead = 0;
    let minSet = false;
...
// is always printed).
export function renderInline(state, data) {
    ...
    if (flash > 0) {
        ...
        return (`${shinyPrefix}${colorCode}${BOLD}${emoji} ${sparkle}${name}${sparkle}${RESET} ` +
    ...
    return (`${shinyPrefix}${colorCode}${BOLD}${emoji} ${name}${RESET} ` +
        `${LEVEL_COLOR}Lv.${level}${RESET} ${colorCode}${xpLabel}${RESET}/${DIM}${nextLabel}${RESET} ` +
        `${gaugeColor}${gaugeFilled}${DIM}${gaugeEmpty}${RESET} ${pctColor}${pct}%${RESET}`);
}
// Port of pokemon_render_sprite_statusline — returns the (trimmed) sprite lines.
export function renderSpriteLines(state, data, deps) {
...
export function renderStatusline(ctx, deps) {
    ...
    if (project !== '')
        out += `  ${BOLD}${CYAN}${project}${RESET}`;
    if (branch !== '')
        out += ` ${YELLOW}${branch}${RESET}`;
    if (model !== '')
        out += `  ${BRIGHT_MAGENTA}${model}${RESET}`;
    if (effort !== '') {
        let label;
        let color;
        switch (effort) {
