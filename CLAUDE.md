...
- **PR workflow** (à partir de 2026-05-12) : feature branches off `main`, **rebase-merge** (historique linéaire — le ruleset impose `linear history` ; pas de squash). 1 PR par gros morceau. `main` branch-protected (20 checks requis). Pre-push hook Claude Code lance les CI gates avant chaque push.
...
4. **Threshold curve** (`lib/data/thresholds.json`, force-propagée via update.sh, rééquilibrée en v1.0.0-beta.7) : ratio géométrique sur les **deltas** (pas sur le cumul — l'ancienne spec se trompait là-dessus et créait une chute de delta à Lv.16). 3 segments :
   - **Œuf** : Lv.0→1 = 1M XP one-shot (~1.7j normal dev, narrativement = hatching cost, non-monotone par design)
   - **Phase A** Lv.1→16 : delta[1→2]=202K, ratio 1.143× par niveau. Lv.5≈2M, Lv.10≈4.3M, Lv.16=10M (~2.4 sem normal dev)
   - **Phase B** Lv.16→100 : ratio 1.021× par niveau. Lv.36≈43M, Lv.55≈90M, Lv.100=312M (~1.4 an normal dev)
   - **Deltas monotones croissants** Lv.1→100 par construction. Sans daily cap, la courbe seule gère la difficulté — heavy devs progressent naturellement plus vite, casual plus lentement.
